...
class UsersViewSetTestCase(APITestCase):
    def setUp(self):
        ...
        response = self.client.post(
            "/api/token-auth/",
            data={"username": "testcase", "password": "some_strong_psw123"},
        )
    ...
    def test_user_data_update_by_random_user(self):
        random_user = CustomUser.objects.create_user(
            username="random", password="psw1345123451"
        )
        self.client.force_authenticate(user=random_user)
        data = {
            "username": "testcase123",
            "about": "about test case123",
            "first_name": "Test123",
            "last_name": "Case",
        }
        response = self.client.put("/api/user/1/", data)
    ...
    def test_user_data_update_by_admin(self):
        admin = CustomUser.objects.create_superuser(
            username="admin", password="psw1345123451"
        )
